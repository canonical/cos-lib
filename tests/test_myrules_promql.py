...
class TestAddRulesFromPath(unittest.TestCase):
    def test_each_alert_rule_is_topology_labeled(self):
        ...
        ri.add_promql_path(Path(__file__).resolve().parent / "promql_rules" / "prometheus_alert_rules")
...
class TestAddRuleFromStr(unittest.TestCase):
    ...
    def test_official_rule_add_alerts_from_dict(self):
        ...
        # GIVEN an alert rule
        rules = MyRules()
        rules._groups.setdefault("promql", []).extend(
            rules._parse_and_label(self.rule, group_name="initial")
        )
        # WHEN a rule is added from string in official-rule format
        rules.add_promql_dict(official_rule)
...
class TestFromDictGroupName(unittest.TestCase):
    ...
    def test_single_rule_from_dict(self):
        ...
        groups = rules._parse_and_label(self.single_rule)
        for group in groups:
    ...
    def test_single_rule_from_dict_custom_group(self):
        ...
        groups = rules._parse_and_label(self.single_rule, group_name="foo")
        for group in groups:
    ...
    def test_single_rule_from_dict_custom_group_prefix(self):
        ...
        groups = rules._parse_and_label(self.single_rule, group_name_prefix="foo")
        for group in groups:
    ...
    def test_official_rule_from_dict(self):
        ...
        groups = rules._parse_and_label(self.official_rule)
        for group in groups:
    ...
    def test_official_rule_from_dict_custom_group_prefix(self):
        ...
        rules = MyRules()
        # WHEN processed as string and provided custom group name prefix
        groups = rules._parse_and_label(self.official_rule, group_name_prefix="foo")
        for group in groups:
    ...
    def test_raises_value_error_invalid_rule_format(self):
        ...
        rules = MyRules()
        with self.assertRaises(ValueError) as ctx:
            rules._parse_and_label(invalid_rule)
...
def get_hash(group_name: str) -> str:
    """Extract the hash of the group name when a group name was not provided to _parse_and_label method in the MyRules class.
...
class TestAlertRulesWithOneRulePerFile(unittest.TestCase):
    ...
    def test_non_recursive_is_default(self):
        rules = MyRules(topology=self.topology)
        rules.add_promql_path(self.sandbox.getsyspath("/rules/prom/"))
    ...
    def test_non_recursive_lma_format_loading_from_root_dir(self):
        rules = MyRules(topology=self.topology)
        rules.add_promql_path(self.sandbox.getsyspath("/rules/prom/lma_format/"))
        rules_file_dict = rules.as_dict()
    ...
    def test_non_recursive_official_format_loading_from_root_dir(self):
        rules = MyRules(topology=self.topology)
        rules.add_promql_path(self.sandbox.getsyspath("/rules/prom/prom_format"))
        rules_file_dict = rules.as_dict()
    ...
    def test_alerts_in_both_formats_are_recursively_aggregated(self):
        ...
        """
        rules = MyRules(topology=self.topology)
        rules.add_promql_path(self.sandbox.getsyspath("/rules/prom"), recursive=True)
        rules_file_dict = rules.as_dict()
...
class TestAlertRulesWithMultipleRulesPerFile(unittest.TestCase):
    ...
    def test_load_multiple_rules_per_file(self):
        ...
        rules = MyRules(topology=self.topology)
        rules.add_promql_path(sandbox.getsyspath("/rules"), recursive=False)
    ...
    def test_deeply_nested(self):
        ...
        rules = MyRules(topology=self.topology)
        rules.add_promql_path(sandbox.getsyspath("/rules"), recursive=True)
        rules_file_dict_read = rules.as_dict()
...
class TestInjectAndValidateRules(unittest.TestCase):
    ...
    def test_each_promql_rule_is_topology_labeled(self):
        # GIVEN a PromQL MyRules instance with default and metadata topology
        ri = MyRules(topology=self.topology)
        for assertion in self.metadata_assertions:
            metadata = assertion["metadata"]
            labels = assertion["labels"]
            # WHEN rules are injected with metadata
            result = ri.inject_and_validate(
                rules={
    ...
    def test_each_logql_rule_is_topology_labeled(self):
        # GIVEN a LogQL MyRules instance with default and metadata topology
        ri = MyRules(topology=self.topology)
        for assertion in self.metadata_assertions:
            metadata = assertion["metadata"]
            labels = assertion["labels"]
