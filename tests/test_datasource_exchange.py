...
@pytest.mark.parametrize("meta, invalid_reason", (
        ({
    ...
    class BadCharm(CharmBase):
        def __init__(self, framework: Framework):
            ...
            self.ds_exchange = DatasourceExchange(
                self,
                provider_endpoint='far',
                requirer_endpoint='boo'
            )

    with pytest.raises(UncaughtCharmError, match=invalid_reason) as e:
...
def test_ds_submit():
    ...
    class MyCharm(CharmBase):
        ...
        def __init__(self, framework: Framework):
            ...
            self.ds_exchange = DatasourceExchange(
                self,
                provider_endpoint='foo',
                requirer_endpoint='bar'
            )
...
def test_ds_receive():
    ...
    ds_requirer_in = [{"type": "c", "uid": "3"}, {"type": "a", "uid": "1"}, {"type": "b", "uid": "2"}]
